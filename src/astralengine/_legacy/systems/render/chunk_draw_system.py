...
def system_chunk_render(world: "ECSWorld", dt: float) -> None:
    ...
    mods: ModelMatrixStore = world.store(ModelMatrix)

    chunk_mesh = world.store(Mesh)

    for eid, i_mesh, i_model in Query(world, (Mesh, ModelMatrix)):
        ...
        model = mods.model[i_model]
        centre = mods.centre[i_model]
        
        renderer.draw_mesh(mesh_id, mesh.verts, mesh.indices, model @ centre)
