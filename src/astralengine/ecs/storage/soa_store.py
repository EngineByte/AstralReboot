...
class SoAStore:
    def __init__(self, entity_capacity: int, initial_dense_capacity: int = 1024) -> None:
        ...
        self._sparse_to_dense: npt.NDArray[np.int32] = np.full(
            self.entity_capacity, -1, dtype=np.int32
        )
    ...
    def dense_index(self, eid: EntityId) -> int:
        si = int(entity_index(eid))
    ...
    def add(self, eid: EntityId, component: Any) -> None:
        ...
        if existing != -1:
            self._on_add_dense(existing, component)
            return
        ...
        self._on_add_dense(dense_i, component)
